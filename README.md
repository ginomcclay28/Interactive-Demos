...
- ตัวนับผู้เข้าชมใช้ **hits.sh** ตัวเดียวกับเด็ค `Immersive-Lecture-class-3` — นับทุกครั้งที่เปิดหน้า จากทุกเครื่อง
  ถ้าโหลดไม่ขึ้นจะถอยไป `page-views-api.ratneshc.com` (ตัวนี้กันนับซ้ำจาก IP เดิม) แล้วค่อย localStorage
  ถ้าตกมาถึงขั้นสุดท้ายจริง ๆ ป้ายใต้ตัวเลขจะเขียนว่า “นับเฉพาะเครื่องนี้” ให้เห็นชัด
- มุมขวาล่างหน้า Home มี **build stamp** ไว้เช็คว่าเบราว์เซอร์กำลังเปิดไฟล์เวอร์ชันไหน (กันหลงเพราะแคช)
